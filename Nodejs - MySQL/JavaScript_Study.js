// nodejs - 12. 저자 생성 기능 구현

// pathName === "/author_create"
// 저자 목록 페이지의 create 버튼의 목적지 url을 "/craete"에서 "/author_create"로 변경
// 서버에게 변수 name, profile로 문자열 데이터를 담아서 전송하기 위한 form 태그 구현

// pathName === "/author_create_process"
// SQL문
...
var app = http.createServer(function(request, response){
    ...
    }else if(pathName === "/author_create"){ // 저자 생성 페이지 생성
        author.create(request, response);
    }else if(pathName === "/author_create_process"){ // 저자 생성 페이지 생성
        author.create_process(request, response);
    }else if(pathName === "/author_update"){ // 저자 생성 페이지 생성
        author.create_process(request, response);
    }else if(pathName === "/author_update_process"){ // 저자 생성 페이지 생성
        author.create_process(request, response);
    }else if(pathName === "/author_delete_process"){ // 저자 생성 페이지 생성
        author.create_process(request, response);
    }else{ // not found page
