// nodejs - 8. join을 이용하여 글 생성 기능 구현

// template module에서
// select태그를 생성하는 authorSelect메서드를 생성

// 글 생성 페이지 UI 구현
// url이 "/create"인 페이지에서
// author테이블의 데이터를 불러와서 authors라는 변수에 대입하였다.
// 그리고 authors를 인수값으로 template.authorSelect(authors)를 호출하여
// select태그를 만들어 글쓴이를 선택할 수 있게 하였다.

// 글 생성 기능 구현
// url이 "/create_process"인 페이지에서
// db.query메서드의 인수값을 줄 때
// INSERT INTO topic (title, description, created, author_id) VALUES(?, ?, NOW(), ?), [post.title, post.description, post.author]
// author_id의 값에 해당하는 ?가 post.author로 치환되도록 코딩
