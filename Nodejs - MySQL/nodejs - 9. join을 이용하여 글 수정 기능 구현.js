// nodejs - 9. join을 이용하여 글 수정 기능 구현

// url "/update"에서
// SQL문이 "SELECT * FROM author"인 db.query()를 추가하여 저자를 선택할 수 있게 하였다.

// template module의 authorSelect 메서드에서
// option태그가 selected되는 것을 추가하였다.
// authorSelect 메서드의 두 번째 인자값이 새로 생겼다.
// author_id라는 값이며 현재 수정되고 있는 글의 저자가 누구인지에 대한 데이터이다.

// url "/update_process"에서
// UPDATE topic SET title=?, description=?, author_id=1 WHERE id=?
// UPDATE topic SET title=?, description=?, author_id=? WHERE id=?
// author_id에 대한 값을 줄 수 있도록 하였다.
...
var app = http.createServer(function(request, response){
    ...
    if(pathName === "/"){ // home page로 왔을 때
        ...
        */
        ...
        db.query(`SELECT * FROM topic`, function(error, titles){
            ...
            db.query(`SELECT * FROM author`, function(error2, authors){
                ...
                list = template.makeFileList(titles);
                response.writeHead(200);
                console.log(template.html(list, title,
                    `${contents}`,
                    `
                    <ul id="controlBtnList">
                        <a href="./create" class="controlBtn">Create</a>
        ...
        */
        ...
        // MySQL database code
        db.query(`SELECT * FROM topic`, function(error, titles){ // 글의 제목들데 대한 정보를 담고있는 데이터 titles
            if(error) throw error;
            db.query(`SELECT * FROM topic WHERE id=?`, [queryData.id], function(error2, topic){ // 선택된 글의 정보를 담고있는 데이터 topic
                if(error2) throw error2;
                db.query(`SELECT * FROM author`, function(error3, authors){ // 저자들에 대한 정보를 담고있는 데이터 authors
